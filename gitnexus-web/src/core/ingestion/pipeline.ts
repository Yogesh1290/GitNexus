...
export const runPipelineFromFiles = async (
  ...
  try {
    // Store file contents for code panel and WebContainer execution
    files.forEach(f => fileContents.set(f.path, f.content));

    // Filter out the massive GitNexus dynamic bundle from the Knowledge Graph AST parsing
    let graphFiles = [...files];
    const manifestFile = files.find(f => f.path === 'gitnexus.json' || f.path.endsWith('/gitnexus.json'));
    if (manifestFile) {
      try {
        const manifest = JSON.parse(manifestFile.content);
        if (manifest.type === 'dynamic') {
          // Get the filename being run/fetched
          let bundleName = '';
          if (manifest.bundleUrl) {
            // e.g. https://github.com/.../gitnexus-bundle.js -> gitnexus-bundle.js
            bundleName = manifest.bundleUrl.split('/').pop() || '';
          } else if (manifest.run) {
            bundleName = manifest.run.replace(/^\.\//, '');
          }

          // Strip it from the Graph AST if it exists in the ZIP
          if (bundleName) {
            graphFiles = graphFiles.filter(f => !f.path.endsWith('/' + bundleName) && f.path !== bundleName);
            if (import.meta.env.DEV) console.log(`[GitNexus] Filtered dynamic bundle \${bundleName} from Knowledge Graph.`);
          }
        }
      } catch (e) { }
    }
    ...
    onProgress({
      ...
      stats: { filesProcessed: 0, totalFiles: graphFiles.length, nodesCreated: 0 },
    });
    ...
    const filePaths = graphFiles.map(f => f.path);
    ...
    onProgress({
      ...
      stats: { filesProcessed: graphFiles.length, totalFiles: graphFiles.length, nodesCreated: graph.nodeCount },
    });
    ...
    onProgress({
      ...
      stats: { filesProcessed: 0, totalFiles: graphFiles.length, nodesCreated: graph.nodeCount },
    });

    await processParsing(graph, graphFiles, symbolTable, astCache, (current, total, filePath) => {
    ...
    await processImports(graph, graphFiles, astCache, importMap, (current, total) => {
    ...
    await processCalls(graph, graphFiles, astCache, symbolTable, importMap, (current, total) => {
    ...
    await processHeritage(graph, graphFiles, astCache, symbolTable, (current, total) => {
    ...
    onProgress({
      ...
      stats: {
        filesProcessed: graphFiles.length,
